...
<div class="input-group form-group form-group-margin" style="width: 100%;" ng-show="procurement.ordersId">
    <table border="3" style="width: 100%;border-color: #2dc3e8;">
        ...
        <c:forEach items="${supermarketPrice}" var="supermarketPrice" varStatus="status">
        <tr>
            <td style="text-align: center;">${status.index+1}</td>
            <td style="height: 40px; padding: 0 20px;">${supermarketPrice.name}</td>
            <td style="text-align: center;">${supermarketPrice.price / 100}</td>
        </tr>
        </c:forEach>
    </table>
</div>
