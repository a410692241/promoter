...
/*				supermarketGoodsId : '',
	...
	function list(){
		...
		$grid.jqGrid({
			url : urls.ms+"/goods/goods/Skulist.do",
/*			postData:$scope.search,*/
			pager : "#GoodsSku1Pager",
			multiselect :true,
			colModel : [
/*			            {name:'supermarketGoodsId',label:'主键'}, */
						...
						{name:'barcode',label:'商品条形码',sortable:false},
...
/*			            {name:'status',label:'审核状态',sortable:false,width:50,formatter:function(cellvalue, options, rowObject){
			            ...
			            {name:'authenticationType',label:'认证类型',sortable:false,sortable:false,formatter:function(cellvalue, options, rowObject){
			            	if(cellvalue == "SHARER"){
			            		return "价格分享员";
