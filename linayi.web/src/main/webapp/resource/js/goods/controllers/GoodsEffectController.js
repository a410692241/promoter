...
app.controller('goodsEffectCtrl'/**
     *
     */
    , function ($q, $http, $scope, toaster, goodsEffectService, messager, templateform, $templateCache) {
        ...
        function save($modalInstance, data, $scope) {
            try {
                ...
                $('#goodsSkuInfoForm').form("submit", {
                    url: urls.ms + "/goods/goods/immediatelyAffect.do",
                    onSubmit: function (param) {
                        for (var key in data) {
                            param[key] = data[key]
                        }
         ...
         * @param goodsSkuId
         */
        function effectPriceNow(goodsSkuId) {
            var param = "?goodsSkuId=" + goodsSkuId;
            // 修改商品规格
            window.open(urls.ms + "/goods/goods/toAddSpecifications.do" + param, "修改商品规格", "height=800, width=1000,top=0,left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no")
        }
