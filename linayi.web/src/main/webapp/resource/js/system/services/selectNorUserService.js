...
app.service('selectUserService', [function() {
	...
	function save( options ){
		...
		if(data.qq==null || data.qq==""){
			throw new BusinessException("请输入qq！");
		}
		if(data.email==null || data.email==""){
			throw new BusinessException("请输入email！");
		}
		...
		options.url = urls.ms+"/employee/addUser.do";
		ajax( options );
	}
	
	function getById( options ){
		options.url = urls.ms+"/employee/get.do";
		ajax( options );
	}
	
	function remove( options ){
		options.url = urls.ms+'/employee/delUser.do';
		ajax( options );
	}
	
	function resetPWD( options ){
		options.url = urls.ms+'/employee/setUserPWD.do';
		ajax( options );
	}
	
	function cancelFrozen( options ){
		options.url = urls.ms+'/employee/cancelFrozen.do';
		ajax( options );
	}
